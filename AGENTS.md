...
When the user asks to deploy the app, run:

```bash
/usr/local/bin/ditty-deploy
```

Do not look for GitHub Pages, Vercel, or ad hoc deploy scripts. The
`ditty-deploy` command builds `dist/`, uploads it through Ditty's deployment
API, and prints the public URL. After a successful deploy, report the Public URL.
